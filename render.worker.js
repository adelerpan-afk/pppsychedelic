// render.worker.js - Fragment Shader dengan Crossfade
...
void main() {
    ...
    if (u_loopDuration > 0.0 && loopDuration > 0.0) {
        ...
        // fadeWindow: seberapa besar bagian yang di-crossfade (0.1 = 10% dari durasi)
        float fadeWindow = 0.15; // 15% dari durasi untuk crossfade
        
        // 4. Hitung weight untuk crossfade
        // - Di awal loop (progress 0-15%): fade in dari frame akhir
        // - Di akhir loop (progress 85-100%): fade out ke frame awal
        float fadeWeight;
        ...
        // 5. Ambil 2 frame: current dan next (atau previous)
        // current: posisi sekarang dalam loop
        // next: posisi di awal/akhir loop (untuk crossfade)
        float t_current = loopTime;
        float t_next;
        
        if (progress < fadeWindow) {
            // Fade in: blend dengan frame di akhir loop
            t_next = loopDuration - (fadeWindow - progress) * loopDuration / fadeWindow;
        } else if (progress > 1.0 - fadeWindow) {
            // Fade out: blend dengan frame di awal loop
            t_next = (progress - (1.0 - fadeWindow)) * loopDuration / fadeWindow;
        } else {
            // Middle: tidak perlu blend
            t_next = t_current;
        }
        
        // 6. Compute pattern untuk kedua frame
        vec3 col_current = computePattern(w, t_current);
        vec3 col_next = computePattern(w, t_next);
        
        // 7. Blend dengan crossfade weight
        col = mix(col_current, col_next, 1.0 - fadeWeight);
        
    } else {
        // No loop
        col = computePattern(w, rawTime);
    }
